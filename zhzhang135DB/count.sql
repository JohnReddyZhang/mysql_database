ANALYZE TABLE Allergies, Appliance, ByMethod, Containers, ContainsAllergy, CookingMethod, Dish, DishTaste, Flavor, Ingredient, IngredientTaste, Isle, KitchenWare, SellsIngredient, SellsKitchenWare, Store, Style, UseIngredient, UseWare, Utensil;
-- Method inspired by:
-- https://stackoverflow.com/questions/2692340/mysql-how-to-count-all-rows-per-table-in-one-query
SELECT TABLE_NAME Name, TABLE_ROWS Count
FROM `information_schema`.`tables`
WHERE `table_schema` = 'zhzhang135DB'
AND table_type like '%Table%';

-- The following query shows the result of count (exact), very long but works,
-- and shows in One table
